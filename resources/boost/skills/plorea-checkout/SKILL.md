...
Stated by Plorea, not yet observed: the native-channel session response and its `clientKey`. Unconfirmed: `channel` on `payment-methods/setup/session`, custom-scheme return URLs. Wallets (Apple Pay / Google Pay) depend on Plorea's Adyen account setup and Apple domain verification — ask Plorea.
...
`Plorea::fake()` answers both session endpoints and returns a client key only for native channels. Assert the JSON has exactly `sessionId`, `sessionData`, `clientKey`, `environment`, and `Plorea::assertSent(fn ($r) => $r->input('channel') === 'iOS')`.
