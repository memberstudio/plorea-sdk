...
- **Webhooks are pings, not truth.** Listen for `MemberFlow\Plorea\Events\PaymentStatusUpdated` and re-fetch `Plorea::payments()->status($event->reference)` — never book money based on payload fields. Handle idempotently: webhook, status poll, and the customer's return page can race on the same reference. A scheduler charge arrives as `subscription.charge_succeeded` → `SubscriptionChargeSucceeded` (re-fetch with `Plorea::subscriptions()->find($event->subscriptionId)`); a manual `charge()` arrives as `payment.authorised` → `PaymentStatusUpdated`. Nothing is emitted for setup, cancel or reactivate. Any other `subscription.*` type reaches you through `WebhookReceived` only — the SDK does not invent events for unobserved shapes. Exclude the webhook path (`plorea/webhook` by default) from CSRF verification.
- **Plorea owns the subscription schedule.** Your app stores a card, creates the subscription, and reacts to state — it never triggers the recurring charge. Billing starts immediately unless a trial is set, and `reactivate()` charges again at once rather than resuming the old cadence. Subscription statuses are `active` / `canceled` (US spelling); cancelling sets `accessEndsAt` one interval past the last charge, so gate access on that, not on `canceledAt`. A payment method must be `active` to be chargeable — a `failed` one is terminal, since the setup verification was refused and no card was stored.
