...
- **Merchant details trigger KYC.** `->merchant(orgNr: ..., name: ..., email: ...)` on a payment link starts merchant onboarding for a new org number; the email receives the KYC mail. The link is payable while KYC is pending (payout held in escrow).
